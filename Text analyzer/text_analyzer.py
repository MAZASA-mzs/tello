"""
Modul for analyzing and checking tello_lang code
"""
    ...
    for line in text_list[:]:
        ...
        if cmd not in _ck_dict:
            ...
            if _ck_dict[cmd] != len(args):
                return (
                    0,
                    f"incorrest count of args to {local_cmd} in command {cmd_count}",
                )
            if cmd == "repeat":
                stack.append(0)
            if cmd == "done":
                stack.append(1)
            if len(stack) >= 2 and stack[-1] and not stack[-2]:
                stack.pop()
                stack.pop()
        cmd_count += 1
    if stack:
        return 0, "unmatched cycle"
    return 1, text_list
    ...
    while code:
        ...
        if cmd == "repeat":
            ...
            if call_st[-1][1] <= 0:
                ...
                cmd_pointer = call_st[-1][0]
        else:
            yield " ".join([cmd] + args)

        cmd_pointer += 1
        if cmd_pointer >= text_l:
            return
